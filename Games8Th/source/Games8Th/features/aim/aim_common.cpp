...
// Sticky RCS compensation multiplier - compute_rcs_factor re-rolls
// the compensation ratio per-time in [min,max] so recoil control is never a fixed
// ratio (robotic). Sticky here, same interval language as the rest of humanize.
float s_rcsMul = 1.f;
std::uint64_t s_rcsMulMs = 0;
...
// RCS: compute_rcs_factor rolls the compensation ratio per-ms in
// [rand_min, rand_max] (%). Ours: menu rcs_scale_x/y is the fixed target ratio -
// apply a sticky ?10% human variance around it (never above the menu value =
// never over-compensates past neutral). Refreshes ~100-160ms, 1.0 when the
// humanize slider is off so existing configs behave exactly as before.
float RcsHumanMul()
{
	...
	const std::uint64_t now = NowMs();
	const std::uint64_t interval = 100ull
		+ static_cast<std::uint64_t>((NoiseUnit() * 0.5f + 0.5f) * 60.f);
	if (s_rcsMulMs == 0 || now >= s_rcsMulMs + interval) {
		s_rcsMulMs = now;
		s_rcsMul = std::clamp(1.f - std::fabs(NoiseUnit()) * (0.10f * hum),
			0.90f, 1.f);
	}
	return s_rcsMul;
}
...
// RcsHumanMul: per-time compensation variance (humanize > 0).
bool GetScaledPunch(C_CSPlayerPawn* lp, QAngle_t& out) {
	...
	const float hum = RcsHumanMul(); // 1.0 when humanize off (behavior unchanged)
...
// -- rcs_smooth ------------------------------------------------------------
// Tracks SCALED punch (same units as GetScaledPunch). Aimbot: add/sub same
// GetRcsApplied(). Standalone: view -= step (already scaled).
// sm=0 -> applied snaps to punch, step = full delta.
namespace {
QAngle_t s_rcsApplied{};
}
...
QAngle_t SmoothRcsStep(const QAngle_t& absolutePunch) {
	...
	QAngle_t delta{
		absolutePunch.x - s_rcsApplied.x,
		absolutePunch.y - s_rcsApplied.y,
		0.f
	};
	...
	s_rcsApplied.x += step.x;
	s_rcsApplied.y += step.y;
	s_rcsApplied.z = 0.f;
	return step;
}
...
bool ApplyDeltaRcs(C_CSPlayerPawn* lp) {
	...
	// Already-scaled absolute punch (kPunchScale * menu X/Y) - same as aimbot path
	QAngle_t absScaled{};
	if (!GetScaledPunch(lp, absScaled)) {
		// Not shooting / no punch - keep baseline for next spray, clear lag state
		QAngle_t raw{};
	...
	if (!g_hadPunch) {
		g_oldPunch = absScaled;
		g_hadPunch = true;
		// Snap lag tracker to current so first real delta is small
		s_rcsApplied = absScaled;
		return false;
	}

	// Recovering punch (recoil settling) - track, don't fight recovery
	{
		const float dx = absScaled.x - g_oldPunch.x;
		const float dy = absScaled.y - g_oldPunch.y;
		if (dx < -0.01f && std::fabs(absScaled.x) + 0.02f < std::fabs(g_oldPunch.x)
			&& std::fabs(dy) < 0.02f) {
			g_oldPunch = absScaled;
			s_rcsApplied = absScaled; // keep lag in sync so resume doesn't spike
			return false;
		}
	}

	// Ease applied -> absScaled; step already in camera units
	const QAngle_t step = SmoothRcsStep(absScaled);
	g_oldPunch = absScaled;
	...
	// Counter punch step (no second kPunchScale - already in absScaled)
	view.x -= step.x;
	view.y -= step.y;
	view.z = 0.f;
	view.Normalize();
