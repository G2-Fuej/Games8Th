...
void __fastcall H::hkOverrideView(void* rcx, void* setup) {
	...
	__try {
		...
		// CreateMove carries the server-facing fake angle. Restore the clean
		// camera angle here so first/third-person visuals remain natural.
		if (AntiAim::g_aa_active) {
			angles[0] = std::clamp(AntiAim::g_realPitch, -89.f, 89.f);
			angles[1] = AntiAim::g_realYaw;
			angles[2] = 0.f;
		}
